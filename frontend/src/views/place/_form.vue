<template>
  <form @submit.prevent="submitForm" class="space-y-2">
    <div class="grid grid-cols-2 gap-4">
      <BaseSelect v-model="formData.client_id" :label="t('labels.client')" :options="optionsClient" required/>
      <BaseInput v-model="formData.place" :label="t('labels.place')" required/>
    </div>
...
onMounted(() => {
    ...
    if (props.formUpdate?.id) {
        ...
        formData.value.place        = (!props.formUpdate.place) ? null : props.formUpdate.place;
...
async function create() {
  try {
    ...
    await placeService.create(formData.value);
...
async function edit() {
  try {
    ...
    loading.show();
    // REQUISIÇÃO AO ENDPONT
    await placeService.update(formData.value.id,formData.value);
